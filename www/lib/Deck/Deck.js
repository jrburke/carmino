...
define(function (require, exports, module) {
  var tempDeck, html, deckNode, hasWebKitCalcBug,
      prim = require('prim'),
      dialogTmpl = require('tmpl!./dialog.html'),
      registry = {},
  ...
  Deck.prototype = {
    ...
    toNode: function (html) {
      tempNode.innerHTML = html;
      var node = tempNode.children[0];
      tempNode.innerHTML = '';
      return node;
    },
    ...
    dialog: function (data) {
      ...
      data.dialogId = 'id' + (dialogIdCounter += 1);
      this.dialogs[data.dialogId] = d;

      html = dialogTmpl(data || {});

      this.node.appendChild(this.toNode(html));
    ...
    toCardNode: function (htmlOrNode, href, moduleId, optClass) {
      if (typeof htmlOrNode === 'string') {
        htmlOrNode = this.toNode(htmlOrNode);
      }
    ...
    _onClick: function (evt) {
      ...
      var dialogDeferred, dialogNode,
          dialogId = evt.target.getAttribute('data-dialogid'),
          href = evt.target.href || evt.target.getAttribute('data-href'),
          link = href && parseHref(href),
          target = link && link.target;

      //Handle any dialog cleanup.
      if (dialogId) {
        // Remove the dialog
        dialogNode = document.querySelector('[role="dialog"][data-dialogid="' +
                     dialogId + '"]');
        if (dialogNode) {
        ...
        dialogDeferred = this.dialogs[dialogId];
        if (dialogDeferred) {
      ...
      if (target) {
        if (target.indexOf('!') === 0) {
          ...
          if (target === 'fn' || target === 'fn-back') {
            this.callFn(link.secondaryTarget, evt.target, link.data, evt)
              .then(function () {
                if (target === 'fn-back') {
                  this.back();
                }
    ...
    callFn: function (fn, target, data, evt) {
      var d = prim(),
      ...
      if (modData) {
        require([modData.id], function (mod) {
          if (mod[fn]) {
            prim().start(function () {
              return mod[fn](modData.node,
                             this.makeLocalDeck(target.href, modData.id),
                             data,
                             evt);
            }.bind(this)).then(d.resolve, d.reject);
          } else {
    ...
    _onKeyPress: function (evt) {
      ...
      if (fn) {
        ...
        this.callFn(fn, target, undefined, evt);
      }
    },
